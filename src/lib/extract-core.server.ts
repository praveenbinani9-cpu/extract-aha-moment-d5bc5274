...
async function getVertexAccessToken(): Promise<string> {
  ...
  let rawJson = process.env.GCP_SERVICE_ACCOUNT_JSON;
  const gacPath = process.env.GOOGLE_APPLICATION_CREDENTIALS;
  if (!rawJson && gacPath) {
    try {
      const fs = await import("node:fs");
      rawJson = fs.readFileSync(gacPath, "utf8");
    } catch {
      // ignore
    }
  }
...
async function callGeminiDirect(images: string[], hint?: string): Promise<string> {
  ...
  function buildVertexBody(withSchema: boolean): string {
    ...
    if (withSchema) {
      generationConfig.responseSchema = VERTEX_EXTRACTION_SCHEMA;
    }
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    // Schema rejected — retry once without responseSchema before giving up.
    if (res.status === 400 && useResponseSchema) {
      console.warn("[vertex] responseSchema rejected (400), retrying without schema", {
        rid,
        attempt,
...
async function callGeminiViaLovableGateway(images: string[], hint?: string): Promise<string> {
  ...
  console.log("[lovable-ai] response", { rid, elapsed_ms: Date.now() - startedAt });
  return json.choices?.[0]?.message?.content ?? "{}";
}
...
export async function extractCore(images: string[], hint?: string): Promise<ExtractCoreOutput> {
  ...
  const compressMs = Date.now() - tCompress;
  console.log("[extract] compress", {
    rid,
    ms: compressMs,
    compressed_count: compressed.compressedCount,
    skipped_count: compressed.skippedCount,
    before_bytes: compressed.beforeBytes,
    after_bytes: compressed.afterBytes,
  });
